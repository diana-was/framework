...
class Kissmetrics extends  Model
{
     ...
     */
    protected  function __construct($db,$awsAccessKey,$awsSecretAccessKey,$kissmetricsBucket,$dbprod) {
        parent::__construct();
        $this->db = $db;
        $this->dbprod = $dbprod;
        $this->db->connect();
        $this->kissmetricsBucket = $kissmetricsBucket;
        $this->s3 = new Amazons3_S3($awsAccessKey,$awsSecretAccessKey);
    }
    
    static public function getInstance() {
    	$class = __CLASS__;
        return new $class(func_get_arg(0),func_get_arg(1),func_get_arg(2),func_get_arg(3),func_get_arg(4));
    }
    ...
    {
        $url = Amazons3_S3::getAuthenticatedURL($this->kissmetricsBucket, $fileName, 60, false, false);
        $file = array();
    ...
    private function _getProdUsersToInsert ($lastUser)
    {
        ...
        $result = array();
        $dResults = $this->dbprod->select($query);
        while ($aRow = $this->dbprod->get_row($dResults, 'MYSQL_ASSOC')) {
                $result[] = $aRow;
        }
        $this->dbprod->free_result($dResults);
        $this->dbprod->close();
        $this->db->connect();
        return $result;
    }
